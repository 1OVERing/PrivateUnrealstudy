...
namespace Ronnie
{
	...
	// 회전행렬 역행렬 구하기.
	Matrix4f Matrix4f::InverseRotationMatrix(const Vector3f& rotation)
	{
		...
		Matrix4f inverseMatrix
			= Matrix4f::Transpose(rotX)
			* Matrix4f::Transpose(rotY)
			* Matrix4f::Transpose(rotZ);
	...
	Matrix4f Matrix4f::Orthographic(float width, float height, float zNear, float zFar)
	{
		float W = 2.0f / width;
		float H = 2.0f / height;
		...
		result.Get(0, 0) = W;		result.Get(0, 1) = 0.0f;	result.Get(0, 2) = 0.0f;	result.Get(0, 3) = 0.0f;
		result.Get(1, 0) = 0.0f;	result.Get(1, 1) = H;	    result.Get(1, 2) = 0.0f;	result.Get(1, 3) = 0.0f;
		result.Get(2, 0) = 0.0f;	result.Get(2, 1) = 0.0f;	result.Get(2, 2) = A;       result.Get(2, 3) = 0.0f;
		result.Get(3, 0) = 0.0f;	result.Get(3, 1) = 0.0f;	result.Get(3, 2) = B;		result.Get(3, 3) = 1.0f;
